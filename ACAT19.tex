...
\footnotetext{This figure was obtained using the \texttt{graphVizTree()} function supported by all nodes in \RooFit and \texttt{graphviz}.}
Therefore, the internal linked list was replaced by a \texttt{std::vector}, mostly to speed up iterating these collections. The
\texttt{RooAbsCollection} was further extended with an STL-like interface (\texttt{size, begin, end}) to enable range-based for loops
for ease of use. Forward iteration is sped up by about $20\,\%$, and random access completes in constant time.
\Cref{itBefore,itAfter} compare iterating through a collection with the old and new iterators. The STL-like interface allows to
reduce heap allocations, and replaces while loops (with non-local variables) by range-based for loops. This reduces code clutter, memory
leaks, dangling pointers and variable shadowing. It will also facilitate
generating Python bindings to iterate through \RooFit's collections using Python loops. \Cref{speedupSTL} compares
run times for typical \RooFit workflows\footnote{These are a selection of representative \RooFit tutorials.} between \ROOT 6.16 and 6.18.
...
\Cref{VecSpeed} shows that run times for fitting simple models decreased by
